...
function AIPlanDayCard({
    ...
    const handleFocusSave = () => {
        onPlanChange({ ...day, focus: focusText });
    };

    const handleEditExercise = (ex: Exercise, index: number) => {
        setEditingExercise({ exercise: ex, index });
        setShowExModal(true);
    };

    const handleAddExercise = () => {
        setEditingExercise({ exercise: null, index: null });
        setShowExModal(true);
    };
    ...
    const handleDeleteExercise = (index: number) => {
        Alert.alert("Delete Exercise", "Remove this exercise from the plan?", [
            { text: "Cancel", style: "cancel" },
            {
                ...
                onPress: () => {
                    const updated = { ...day, exercises: day.exercises.filter((_, i) => i !== index) };
                    onPlanChange(updated);
                },
            },
        ]);
                        ...
                        {editMode && (
                            <View style={ps.editControls}>
                                <View style={ps.editControlRow}>
                                    ...
                                    <TouchableOpacity
                                        style={[ps.toggleBtn, isRest && ps.toggleBtnActive]}
                                        onPress={handleToggleRest}
                                    >
                                        <Text style={[ps.toggleBtnText, isRest && ps.toggleBtnTextActive]}>
                                            {isRest ? "Yes" : "No"}
                                        </Text>
                                    </TouchableOpacity>
                                </View>
...
function AIPlanSection({
    ...
    const regenerate = async () => {
        ...
        Alert.alert(
            "Regenerate Plan",
            "This will replace your current plan with a brand new AI-generated one. Continue?",
            [
                { text: "Cancel", style: "cancel" },
                {
        ...
        <View style={ps.wrap}>
            ...
            {loading || regenerating ? (
                <View style={ps.loadingBox}>
                    <Text style={ps.loadingText}>
                        {regenerating ? "⚡ Bernard is crafting your new plan..." : "Loading your plan..."}
                    </Text>
                </View>
            ) : !plan ? (
                <View style={ps.loadingBox}>
...
export default function ExerciseCalendar() {
    ...
    const [newWorkout, setNewWorkout] = useState({
        name: "", type: "strength" as WorkoutEvent["type"], duration: "", time: "",
    });
